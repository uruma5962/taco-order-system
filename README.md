...
本リポジトリは祭りで実運用した FastAPI + Discord 版のコードです。
現在はこれをベースに、**アプリ化を見据えた SPA 版**（GUI 商品管理、トッピング差額対応、営業日締めなど）を別途制作中です。
ソースコード本体はアプリ化（プロダクト化）を見据えて非公開ですが、参考までに開発中の主要画面を以下に掲載します。
